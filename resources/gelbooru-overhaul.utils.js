...
class RepeatFetchQueue {
	...
	checkNext() {
		while (
			...
			this.activeItems.length < this.parallelRequests
		) {
...
class utils {
	...
	static getPageType() {
		...
		if (!params.has("page")) return utils.pageTypes.UNDEFINED;
	...
	static debugLog(message, value, force = false) {
		if (force || context.configManager.config.general.items.debug.value) {
			if (!value) console.log("[GELO]: " + message);
			else if (!message) console.log("[GELO]: Outputs value", value);
			else console.log("[GELO]: " + message, value);
		}
	}
	/**
			...
			case utils.pageTypes.POST:
				...
				return [
					...document.querySelectorAll(
						".mainBodyPadding > div > div"
					),
				]
					.filter((i) => i.textContent == text)[0]
					.parentElement.querySelectorAll("a > img");
			default:
				return undefined;
		}
	}
	...
	static getThumbPostId(thumb) {
		return Number(
			/id=([0-9]+)/.exec(thumb.parentElement.getAttribute("href"))[1]
		);
	}
	/**
	 * @typedef PostItem
	 ...
	 */
	static GM_setValueThrottle = utils.throttle(
		(v) => GM_setValue("postCache", v),
		1000
	);
	...
	static async loadPostItem(postId) {
		...
		if (
			Object.keys(utils.postCache).length >
			context.configManager.findValueByKey("general.maxCache")
		)
			utils.postCache = {};
			...
			return this.fetchQueue
				.Fetch(
					"https://" +
						...
						"/index.php?page=dapi&s=post&q=index&json=1&api_key=" +
						apiKey +
						"&user_id=" +
						userID +
						"&id=" +
				...
				.then((json) => {
					...
					let highResThumb = fileLink.startsWith("https://video")
						? fileLink
								.replace(
									new RegExp(/\.([^\.]+)$/, "gm"),
									".jpg"
								...
								.replace(
									/https:\/\/[^\.]+\./,
									"https://givemeasubdomainslayer."
								)
						: fileLink;
					let md5 = post.md5;

					if (!highResThumb || !fileLink)
		...
		} else return utils.postCache[postId];
	}
	/**
	 *
	 * @param {PostItem} post
	...
	static downloadPostItem(post) {
		return new Promise((resolve, reject) => {
			//build filename
			let filename = String(
				context.configManager.findValueByKey("fastDL.pattern")
			);
			let spr = String(
				context.configManager.findValueByKey("fastDL.separator")
			);
			...
			filename = filename.replace(
				...
				post.tags.artist.length
					? post.tags.artist.join(spr)
					: "unknown_artist"
			);
			filename = filename.replace(
				"%character%",
				post.tags.character.length
					? post.tags.character.join(spr)
					: "unknown_character"
			);
			filename = filename.replace(
				"%copyright%",
				post.tags.copyright.length
					? post.tags.copyright.join(spr)
					: "unknown_copyright"
			);
			...
			if (context.configManager.findValueByKey("fastDL.saveTags")) {
				let text =
					post.tags.artist.map((i) => "artist:" + i).join("\n") +
					"\n" +
					post.tags.character
						.map((i) => "character:" + i)
						.join("\n") +
			...
			utils.debugLog("Downloading started", {
				url: post.download,
				filename,
			});
		});
	...
	static downloadPostById(postId) {
		loadPostItem(postId)
			.then((p) =>
				downloadPostItem(p)
					.then(() =>
						utils.debugLog("Post item successfully downloaded", p)
					)
					.catch((r) =>
						utils.debugLog("Failed to download post item", {
							post: p,
							error: r.error,
							details: r.details,
						})
			...
			.catch((e) =>
				utils.debugLog("Failed to load post item for", {
					post: e.target,
					id: postId,
					error: e,
				})
